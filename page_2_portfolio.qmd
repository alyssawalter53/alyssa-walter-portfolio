...
total_landings_year %>%
  ...
  scale_y_continuous(
    name = "Total Pounds")  #add second y axis label +
  labs(
    title = "Total US Commercial Shark Landings Over Time",  # plot title
    x = "Year",  # x-axis label
    y = "Total Pounds",  # primary y-axis label
    caption = "Source: NOAA Fisheries landings data"  # data source caption
  ) +
  theme_minimal()  # use minimal theme for cleaner look

# alternative version with formatted y-axis and more y-axis breaks
total_landings_year %>%
  ggplot() +
  geom_line(aes(x = year, y = total_pounds), color = "steelblue") + # plot total pounds on the y axis and year on the x axis
  geom_line(aes(x = year, y = total_dollars), color = "darkgreen") +  # plot second line with total dollars on the y axis and year on the x axis
